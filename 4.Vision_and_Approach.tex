...
representation~\cite{txa-ambrus-defining}, which ensures that only
...
(DCwFs) as developed in~\cite{Altenkirch,Neumann}. We will address the
...
\item \textbf{Extension to lax ends and coends:} We will extend the
  ...
  inspired by recent work including~\cite{Paige}.  
...
$\omega$-categorical setting, we must drop this uniqueness assumption
...
\item \textbf{Canonicity:} Every closed term reduces to a value. This
  ...
  interpretation.
\item \textbf{Normalization:} Every term reduces to a normal
...
types presents significant challenges, but we aim to build on ideas
from the parametricity calculus, which shares key structural
similarities.
...
\paragraph{WP 5: Prototypical Implementation}
...
results established in WP~3, such as normalization and
...
parametricity can be formally derived within DTT — either in general
or in specific fragments. We will explore how directed type structure
can encode uniform behavior over morphisms and equivalences, and
whether certain kinds of logical relations (e.g., bridge/path
structures) can be internalized. We will also extend this
investigation to higher-dimensional settings, building on the semantic
foundations laid in WP~2.
...
quotients~\cite{shulmann-lumsdene}. This raises the question: what
kind of generalised W-types (QW-types) serves as a universal encoding
for QITs or, more broadly, QIITs?
